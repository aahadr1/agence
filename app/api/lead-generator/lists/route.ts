...
export async function POST(request: Request) {
  try {
    ...
    const { name, keywords, leadIds, searchContext } = await request.json();
    if (!name) return NextResponse.json({ error: "Name is required" }, { status: 400 });

    const serviceClient = await createServiceClient();
    const orgId = await resolveOrgIdForUser(serviceClient, user.id);
    const normalizedContext = normalizeSearchContext(searchContext, {
    ...
    const { data: list, error } = await serviceClient
      ...
      .insert({
        ...
        name,
        keywords: keywords || [],
        search_context: createSearchContext(normalizedContext),
      })
      .select()
      .single();
