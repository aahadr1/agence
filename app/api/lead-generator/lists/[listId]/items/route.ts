...
import { mergeSearchContext } from "@/lib/lead-agent/search-context";
...
export async function POST(
  ...
  try {
    ...
    const { leadIds, keywords, searchContext } = await request.json();
    ...
    const { data: list } = await serviceClient
      .from("lead_lists")
      .select("excluded_business_names, keywords, search_context")
      .eq("id", listId)
      .eq("org_id", orgId)
      .single();
    ...
    if (leads) {
      ...
      const mergedKeywords = [
        ...new Set([...(list.keywords || []), ...((keywords as string[]) || [])]),
      ];
      const mergedContext = mergeSearchContext(list.search_context, {
        attempted_keywords: mergedKeywords,
        attempted_queries: Array.isArray(searchContext?.attempted_queries)
          ? searchContext.attempted_queries
          : [],
        successful_queries: Array.isArray(searchContext?.successful_queries)
          ? searchContext.successful_queries
          : [],
        last_generated_queries: Array.isArray(searchContext?.last_generated_queries)
          ? searchContext.last_generated_queries
          : [],
        niche:
          typeof searchContext?.niche === "string" ? searchContext.niche : undefined,
        location:
          typeof searchContext?.location === "string"
      ...
      await serviceClient
        .from("lead_lists")
        .update({
          excluded_business_names: merged,
